...
ggplot() +
  ...
  labs(title = "Cleaning products Consumed per capita",
       subtitle = "",
       x = "", y = "") + theme_void() +
  theme(plot.title=element_text(size=15,face="bold",hjust=1.2))  
...
ggplot() +
  ...
  coord_fixed() + scale_fill_distiller(palette="YlOrRd") +
  labs(title = "Total household cleaning products consumed",
       subtitle = "",
       x = "", y = "") + theme_void() +
